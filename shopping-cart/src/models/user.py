...
async def create_user(users_collection, user):
    try:
        ...
        if user.inserted_id: #busca o id que acabei de criar
            user = await get_user(users_collection, user.inserted_id) #funçao para buscar usuario e retornar
            return user
...
async def get_users(users_collection, skip, limit): #paginação
    try:
...
async def update_user(users_collection, user_id, user_data):
    try:
        data = {k: v for k, v in user_data.items() if v is not None} #validação
